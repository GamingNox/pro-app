        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link key={tab.href} href={tab.href} className="flex flex-col items-center justify-end gap-1 w-14 relative pb-2.5 pt-2.5">
              ...
              <span className={`text-[7px] leading-none tracking-wider transition-all duration-300 ${isActive ? "text-accent font-bold" : "text-muted font-medium"}`}>
                {tab.label}
              </span>
              {isActive && (
                <motion.div layoutId="client-nav-indicator" className="absolute top-0 w-5 h-[2.5px] rounded-full bg-accent"
