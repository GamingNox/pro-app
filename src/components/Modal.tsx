...
export default function Modal({ open, onClose, title, children, size = "default" }: ModalProps) {
  ...
  // Lock body scroll completely when modal opens
  useEffect(() => {
    if (!open) return;
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            className={`relative z-10 w-full bg-white rounded-t-[28px] sm:rounded-[28px] flex flex-col sm:mb-0 shadow-apple-lg ${
