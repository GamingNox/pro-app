...
// Usage: SUPABASE_URL=... SUPABASE_KEY=... node scripts/seed-remaining.mjs
...
async function seed() {
  ...
  const { data: clientsList } = await supabase.from("clients").select("id, first_name").eq("user_id", userId);
  const cIds = {};
  clientsList.forEach((c) => { cIds[c.first_name] = c.id; });
  console.log("Clients:", Object.keys(cIds).join(", "));
